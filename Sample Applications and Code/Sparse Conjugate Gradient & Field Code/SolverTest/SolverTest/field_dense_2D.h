...
#ifndef DROGON_FIELD_DENSE_2D_H
#define DROGON_FIELD_DENSE_2D_H
...
namespace drogon {
	...
	//! 2d Field dense field
	//! Internal indexing is size_t, Local and World access are float.
	template<class T_DATA>
	class field_dense_2D : public field_base_dense<T_DATA, float>
	{
	public:
		//! ctor
		field_dense_2D(string name, FIELD_EXTENT data_window, FIELD_EXTENT display_window, FIELD_EXTENT simulation_window) : field_base_dense((data_window.maxX() - data_window.minX()) * (data_window.maxY() - data_window.minY()), name, data_window, display_window, simulation_window)
		{
		...
		void copy_from(const field_dense_2D& other, bool run_parallel)
		{
			m_DATA = other.m_DATA;
		}
